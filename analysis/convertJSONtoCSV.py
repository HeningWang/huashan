...
filenames = glob.glob("..\\data\\pfaender*.json")

header = ""
data = []

#open output file and initialize csv writer for data
write_file = open('..\\data\\pfaender.csv', 'w', newline='')
writer = csv.writer(write_file)

#open output file and initialize csv writer for participant info
write_subj_info_file = open('..\\data\\pfaender_subj_info.csv', 'w', newline='')
...
for file in filenames:
    id = re.split("pfaender",(re.split("\.json", file)[0]))[1]
    #print(id);
